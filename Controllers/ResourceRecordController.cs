    ...
    IMeilisearchService meilisearchService) : Controller
{
        ...
        [FromQuery] ResourceRecordFilterDto? filterDto = null)
    {
        ...
        if (dto.Search != null)
        {
            ...
            resourceRecords =
                (await resourceRecordRepository.GetResourceRecordsAsync(predicate: e => idList.Contains(e.Id)))
    ...
    public async Task<IActionResult> CreateResourceRecord([FromBody] ResourceRecordRequestDto dto)
    {
        ...
        return StatusCode(StatusCodes.Status201Created,
            new ResponseDto<CreatedResponseDto<Guid>>
            {
                Status = ResponseStatus.Ok,
                Code = ResponseCodes.Ok,
                Data = new CreatedResponseDto<Guid> { Id = resourceRecord.Id }
            });
    }
    ...
    public async Task<IActionResult> CreateResourceRecords([FromBody] List<ResourceRecordRequestDto> dtos)
    {
        ...
        return StatusCode(StatusCodes.Status201Created,
            new ResponseDto<CreatedResponseDto<IEnumerable<Guid>>>
            {
                Status = ResponseStatus.Ok,
                Code = ResponseCodes.Ok,
                Data = new CreatedResponseDto<IEnumerable<Guid>> { Id = resourceRecords.Select(e => e.Id) }
            });
    }
