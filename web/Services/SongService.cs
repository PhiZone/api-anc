    ...
    IHostEnvironment env) : ISongService
{
    ...
    public async Task<(string, string, TimeSpan)?> UploadAsync(string fileName, string filePath)
    {
        var stream = await multimediaService.ConvertAudio(filePath);
        if (stream == null) return null;
        try
        {
            var result = await fileStorageService.Upload<Song>(fileName, stream, "ogg");
        ...
        string? filePath = null)
    {
        ...
        await natsService.GetClient()
            .PublishAsync(_queue,
                new SongTaskDto
                {
                    SongId = songId, IsSubmission = isSubmission, Burn = burn, Body = body!, FilePath = filePath
                });
    }
}