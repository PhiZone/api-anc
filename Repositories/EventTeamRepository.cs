    ...
    : IEventTeamRepository
{
    public async Task<ICollection<EventTeam>> GetEventTeamsAsync(List<string>? order = null, List<bool>? desc = null,
        int? position = 0, int? take = -1, Expression<Func<EventTeam, bool>>? predicate = null,
        int? currentUserId = null)
    {
        var result = context.EventTeams.Include(e => e.Participations.OrderBy(f => f.DateCreated))
    ...
    {
        IQueryable<EventTeam> result = context.EventTeams.Include(e => e.Participations.OrderBy(f => f.DateCreated))
            .ThenInclude(e => e.Participant)
            .ThenInclude(e => e.Region);
        if (currentUserId != null)
            result = result.Include(e => e.Likes.Where(like => like.OwnerId == currentUserId).Take(1));
